...
void USBInterrupt(void) { // inline not really working in multiple files in SDCC
  ...
  if (UIF_BUS_RST) {
    ...
    UsbConfig = 0;
    UpPoint1_Busy = 0;  // Reset EP1 busy flag — bus reset interrupts in-flight transfers,
                        // EP1_IN callback never fires, flag would stay 1 forever otherwise.
...
#else
  UDEV_CTRL = bUD_PD_DIS;    // Disable DP/DM pull-down resistor (CH551/552/549)
#endif
